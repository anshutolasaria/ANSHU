...
// Turns a rough note into a short, broad Google News search query.
async function buildNewsQuery(note) {
  const query = await generate({
    system: [
      ...
      'Reply with one broad search query of 2-3 plain keywords naming the main topic of the note, most important word first (for example: sunscreen SPF India, or niacinamide serum).',
      'No quotes, no operators, no explanation.',
    ].join('\n'),
    user: note,
    temperature: 0,
    timeoutMs: 8_000,
...
async function draftPost(note, newsItems = []) {
  const system = [
    ...
    '- Reference at least one headline if any is relevant to the note, and never force in one that is not.',
  ...
  return generate({ system, user, temperature: 0.8, timeoutMs: 40_000 });
}
